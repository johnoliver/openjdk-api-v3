...
data class GHAssets @JsonCreator constructor(@JsonProperty("nodes") val assets: List<GHAsset>,
    ...
    private fun assetToBinary(asset: GHAsset, metadata: Map<GHAsset, GHMetaData>, assets: List<GHAsset>): Deferred<Binary> {
        return GlobalScope.async {
            ...
            if (binaryMetadata != null) {
                return@async binaryFromMetadata(binaryMetadata, pack, download_count, updated_at, installer, heap_size)
            } else {
    ...
    private fun getInstaller(asset: GHAsset, assets: List<GHAsset>): Installer? {
        ...
        val nameWithoutExtension = BINARY_ASSET_WHITELIST.fold(asset.name, { assetName, extension -> assetName.replace(extension, "") })
    ...
    private fun binaryFromName(asset: GHAsset, pack: Package, download_count: Long, updated_at: LocalDateTime, installer: Installer?, heap_size: HeapSize): Binary {
        val scm_ref = null
    ...
    private fun binaryFromMetadata(binaryMetadata: GHMetaData, pack: Package, download_count: Long, updated_at: LocalDateTime,
                                   installer: Installer?, heap_size: HeapSize): Binary {
    ...
    private fun getChecksum(binary_checksum_link: String?): String? {
        if (binary_checksum_link != null && binary_checksum_link.isNotEmpty()) {
            ...
            val checksum = HttpClientFactory.getHttpClient().sendAsync(request, HttpResponse.BodyHandlers.ofString()).get()

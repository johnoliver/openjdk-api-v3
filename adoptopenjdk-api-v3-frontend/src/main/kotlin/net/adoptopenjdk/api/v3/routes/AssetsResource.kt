...
class AssetsResource {
    ...
    @GET
    @Path("feature_releases/{release_type}/{version}")
    ...
    fun get(
            ...
            @Parameter(name = "version", description = "Feature release version e.g. 8,11,13.", required = true,
        ...
        val releases = getFilteredReleases(listOf(repos), releaseFilter, binaryFilter)
    ...
    fun getReleaseVersion(
        ...
        val releaseFilter = ReleaseFilter(ReleaseType.ga, null, null, vendor, range)
        val binaryFilter = BinaryFilter(os, arch, image_type, jvm_impl, heap_size)

        val repos = APIDataStore.getAdoptRepos().repos.values
        val releases = getFilteredReleases(repos, releaseFilter, binaryFilter)
        return getPage(pageSize, page, releases)
    }
    ...
    private fun getFilteredReleases(releases: Collection<FeatureRelease>, releaseFilter: ReleaseFilter, binaryFilter: BinaryFilter): Sequence<Release> {
        return releases
                .asSequence()
                .flatMap { it.getReleases(releaseFilter) }
                .map { release ->
